...
def test_elemental_storm_preset_generates_valid_frames() -> None:
    options = RenderOptions(
        input_audio=Path("song.wav"),
        output_video=Path("out.mp4"),
        preset="elemental_storm",
        width=96,
        height=72,
        fps=2,
        max_seconds=1,
    ...
    scalar = TimeSeries(times=[0.0, 1.0], values=[0.25, 0.9])
    vector = TimeSeries(times=[0.0, 1.0], values=[[0.2, 0.8, 0.4, 0.9], [0.9, 0.3, 0.7, 0.2]])
    analysis = AudioAnalysis(
        duration_seconds=1.0,
        sample_rate=22_050,
        tempo_bpm=120.0,
    ...
    frames = list(
        generate_reactive_frames(options, 1.0, Timeline(analysis), load_preset("elemental_storm"))
    )
...
def test_inferno_portal_preset_generates_valid_frames() -> None:
    options = RenderOptions(
        input_audio=Path("song.wav"),
        output_video=Path("out.mp4"),
        preset="inferno_portal",
        width=96,
        height=72,
        fps=2,
        max_seconds=1,
    ...
    scalar = TimeSeries(times=[0.0, 1.0], values=[0.3, 0.95])
    vector = TimeSeries(times=[0.0, 1.0], values=[[0.2, 0.9, 0.5, 0.7], [0.9, 0.4, 0.8, 0.3]])
    analysis = AudioAnalysis(
        duration_seconds=1.0,
        sample_rate=22_050,
        tempo_bpm=120.0,
    ...
    frames = list(
        generate_reactive_frames(options, 1.0, Timeline(analysis), load_preset("inferno_portal"))
    )
